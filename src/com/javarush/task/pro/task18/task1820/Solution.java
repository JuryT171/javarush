...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        ...
        return cars.max((c1,c2)->c1.getPrice()- c2.getPrice());  // метод макс ищет максимальную цену,сравнивая два обьекта
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        //напишите тут ваш код
        return cars.filter(c->c.getPrice()>mostExpensiveCar.getPrice()).findFirst();  // с помощью фильтра оставляем авто дороже чем mostExpCars,
        // цена больше чем самая дорогая тесла, берем первый из подходящих под условия
    }
}
